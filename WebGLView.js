...
class WebGLView extends Widget {
    ...
    constructor( idPrefix, parent, x = 0.0, y = 0.0, width = 400.0, height = 400.0 ) {
        ...
        this.glCanvas.style[ 'border-radius' ] = "4x 4px";
        this.glCanvas.style[ 'background-color' ] = 'rgb( 40, 40, 40 )';
        ...
        this.canvas2d.style[ 'border-radius' ] = "10px 10px";
    ...
    // --- A1
    printTickAnnotation(intensity, frequency, xtick, ytick){
        this.printXaxisAnnotation(intensity, xtick);
        this.printYaxisAnnotation(frequency, ytick);
    }

    printXaxisAnnotation(intensity, xtick){
        var ctx = this.ctx;
        ctx.save();
        ...
        ctx.fillText(intensity.label, intensity.x, intensity.y);

        // Tick
        ctx.textAlign = "center";
        ctx.fillStyle = "white";
        ctx.font = "12px serif";
        ctx.fillText(xtick.labelLeft, xtick.xLeft, xtick.yLeft);
        ctx.fillText(xtick.labelRight, xtick.xRight, xtick.yRight);

        ctx.restore();
    }

    printYaxisAnnotation(frequency, ytick){
        var ctx = this.ctx;
        ...
        ctx.fillStyle = "white";
        ctx.font = "bold 18px serif";
        ctx.translate(frequency.x, frequency.y);
        ctx.rotate(-Math.PI / 2.0);
        ctx.fillText(frequency.label, 0, 0);
        ctx.restore();
        ...
        ctx.fillStyle = "white";
        ctx.font = "12px serif";
        ctx.translate(ytick.xBottom, ytick.yBottom);
        ctx.rotate(-Math.PI / 2.0);
        ctx.fillText(ytick.labelBottom, 0, 0);
        ctx.restore();
        ...
        ctx.fillStyle = "white";
        ctx.font = "12px serif";
        ctx.translate(ytick.xTop, ytick.yTop);
        ctx.rotate(-Math.PI / 2.0);
        ctx.fillText(ytick.labelTop, 0, 0);
        ctx.restore();
    }


    printHoverInteractionAnnotation(frequency, probability){
        var ctx = this.ctx;

        // clear last annotation first 
        ctx.save();
        ctx.clearRect(frequency.x - this.canvas2d.width*0.2 / 1.3, frequency.y - this.canvas2d.height*0.08/1.4, this.canvas2d.width*0.7 / 1.3, this.canvas2d.height*0.15/1.4);

        // annotation frequency
        ctx.textAlign = "center";
        ctx.fillStyle = "white";
        ctx.font = "14px serif";
        ctx.fillText(frequency.text, frequency.x, frequency.y);
        ctx.restore();
        ...
        ctx.fillStyle = "white";
        ctx.font = "14px serif";
        ctx.fillText(probability.text, probability.x, probability.y);
        ctx.restore();

    }

    drawHoverInteractionRectangle(rectData){
        var ctx = this.ctx;

        // clear last rectangle first
        ctx.save();
        ctx.clearRect(this.canvas2d.width*0.1/1.3, this.canvas2d.height*0.19/1.4, 
                      this.canvas2d.width*1.0/1.3, this.canvas2d.height*1.0/1.4);
        ctx.restore();

        // draw hovered interaction rectangle
        ctx.save();
        ctx.beginPath();
        ctx.fillStyle = "#FF0000";
        ctx.fillRect(rectData.startX, rectData.startY, rectData.width, rectData.height);
        ctx.stroke();
        ctx.restore();

        this.lastHoveredRectangle = rectData;
    }
    // --- A1
